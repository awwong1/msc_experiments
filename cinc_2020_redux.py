...
def main():
    for dc_idx, data_config in enumerate(data_configs):
        ...
        with ElapsedTimer() as t:
            ...
            with open(
                os.path.join(experiment_dir, f"v{dc_idx}_test_results.json"), "w"
            ) as f:
                json.dump(log, f)
            with open(
                os.path.join(experiment_dir, f"v{dc_idx}_test_class_labelwise.csv"), "w"
            ) as f:
                f.write(class_output_string)
            with open(os.path.join(experiment_dir, f"v{dc_idx}_models.pkl"), "wb") as f:

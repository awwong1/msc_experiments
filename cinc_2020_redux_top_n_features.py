...
def train_experiment(
    ...
    train_features, train_labels = features.take(train_idxs), np.take(
        root["raw/dx"], train_idxs
    )
    eval_features, eval_labels = features.take(val_idxs), np.take(
        root["raw/dx"], val_idxs
    )
    test_features, test_labels = features.take(test_idxs), np.take(
    ...
    for idx_sc, sc in enumerate(scored_codes):
        with ElapsedTimer() as t:
            ...
            _train_features = train_features.iloc[:, top_n_feat_idxs]
            _eval_features = eval_features.iloc[:, top_n_feat_idxs]
            _test_features = test_features.iloc[:, top_n_feat_idxs]
...
def main():
    ...
    for dc_idx, data_config in enumerate(data_configs):
        ...
        with ElapsedTimer() as t:
            with open(os.path.join(experiment_dir, f"v{dc_idx}_models.pkl"), "rb") as f:
                old_experiment = pickle.load(f)

            log, class_output_string, models = train_experiment(data_config, old_experiment, top_n=top_n)

            with open(
                os.path.join(experiment_dir, f"top_{top_n}", f"v{dc_idx}_test_results.json"), "w"
            ) as f:
                json.dump(log, f)
            with open(
                os.path.join(experiment_dir, f"top_{top_n}", f"v{dc_idx}_test_class_labelwise.csv"), "w"
            ) as f:
                f.write(class_output_string)
            with open(os.path.join(experiment_dir, f"top_{top_n}", f"v{dc_idx}_models.pkl"), "wb") as f:
                pickle.dump({**models, "data_config": data_config}, f)

        print(os.path.join(experiment_dir, f"top_{top_n}", f"v{dc_idx}_models.pkl"))
